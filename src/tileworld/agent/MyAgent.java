...
public class MyAgent extends TWAgent {
  ...
  public MyAgent(String name, int xpos, int ypos, TWEnvironment env, double fuelLevel) {
    ...
    this.pathGenerator = new AstarPathGenerator(env, this, Parameters.lifeTime);
  ...
  protected TWThought think() {
    if (needRefuel()) {
      ...
      if (fuelStation == null) {
        System.out.println("Unlucky! Don't know where to refuel!");
        return null;
      }
  ...
  private boolean needRefuel() {
    TWFuelStation fuelStation = ((MyMemory) getMemory()).getFuelStation();
    return fuelStation != null && fuelStation.getDistanceTo(this) * 1.5 > this.getFuelLevel();
  }
  ...
  public void communicate() {
    ...
    Message message = new MyMessage("", "", "", entities);
